...
assert.match(colorSamplePrompt, /吸取颜色不是默认改文本色/);
assert.match(colorSamplePrompt, /颜色应用方式: 自动判断/);
assert.match(colorSamplePrompt, /background-color \/ background \/ color \/ border-color/);
assert.match(colorSamplePrompt, /优先考虑背景色或组件主题色/);
...
const backgroundColorPrompt = buildColorSamplePrompt(colorSamples, "Codex", {
  targetReference,
  applyMode: "background"
});
assert.match(backgroundColorPrompt, /颜色应用方式: 背景色/);
assert.match(backgroundColorPrompt, /优先修改 background-color、background、组件背景变量或主题主色/);
assert.match(backgroundColorPrompt, /不要把它当作文本色/);
